 ...
 */
static void chooseIndicesAtRandom(register int *indices, register int n,
                                  register int k, bool tryRandom) {
 ...
 *
 **/
void FuzzyVaultTools::chooseFiniteFieldSetAtRandom(uint32_t *x, int n,
...
FuzzyVaultTools::createRandomInstance(uint32_t *x, uint32_t *y, int n, int t,
                                      int k, const SmallBinaryField &gf,
                                      bool tryRandom) {
 ...
 */
SmallBinaryFieldPolynomial
FuzzyVaultTools::createRandomInstance(SmallBinaryFieldPolynomial &V, int t,
                                      int k, bool tryRandom) {
 ...
 *            see 'FuzzyVaultTools.h'
 */
bool FuzzyVaultTools::bfattack(SmallBinaryFieldPolynomial &f, const uint32_t *x,
                               const uint32_t *y, int n, int k,
                               const uint32_t hash[5], uint64_t maxIts) {
  ...
  if (n > RAND_MAX) {
    cerr << "FuzzyVault::bfattack: The number of vault points must be"
         << " smaller than or equal RAND_MAX which is" << RAND_MAX << "."
         << endl;
    exit(EXIT_FAILURE);
  }
  ...
  for (uint64_t it = 0; it < maxIts; it++) {
    ...
    sha.hash(candidateHash, candidatePolynomial.getData(),
             candidatePolynomial.deg() + 1);
...
bool FuzzyVaultTools::bfattack(SmallBinaryFieldPolynomial &f, const uint32_t *x,
                               const uint32_t *y, int n, int k,
                               const uint8_t hash[20], uint64_t maxIts) {
 ...
 *            see 'FuzzyVaultTools.h'
 */
bool FuzzyVaultTools::bfdecode(SmallBinaryFieldPolynomial &f, const uint32_t *x,
                               const uint32_t *y, int n, int k,
